...
const createMessage = async () => {
  ...
  //1. 입력된 메세지에서 http로 시작하는 문장이 있는지 먼저 찾기 (.find())등의 알고리즘 사용
  ...
  //3. 스크래핑 결과 (result) 에서 OG(openGraph) 코드 골라내서 변수에 저장하기
  const $ = cheerio.load(result.data);
  $("meta").each((i, el) => {
    if ($(el).attr("property") && $(el).attr("property").includes("og:")) {
      const key = $(el).attr("property"); //ex) og: title, og:description
